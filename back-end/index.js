...
app.get('/api/users', (req, res) => {
  ...
  try {
    ...
    if (decoded.role === 'admin') {
      ...
      res.status(403).json({ message: 'Only admin can access user list' });
    }
  } catch (error) {
    res.status(401).json({ message: 'Invalid token' });
  }
...
app.get('/api/users/:username/status', (req, res) => {
  ...
  try {
    ...
    if (decoded.role === 'admin') {
      ...
      res.status(403).json({ message: 'Only admin can access user status' });
    }
  } catch (error) {
    res.status(401).json({ message: 'Invalid token' });
  }
...
app.put('/api/users/:username/status', (req, res) => {
  ...
  try {
    ...
    if (decoded.role === 'admin') {
      ...
      res.status(403).json({ message: 'Only admin can update user status' });
    }
  } catch (error) {
    res.status(401).json({ message: 'Invalid token' });
  }
